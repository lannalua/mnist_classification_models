...
def train_evaluate(lr, dropout_rate):
...
def otimizar_lr(dropout_fixo):
    print(f"\n--- [1/3] Buscando LR (Fixo: Drop={dropout_fixo}) ---")
...
def otimizar_dropout(lr_fixo):
    print(f"\n--- [2/3] Buscando Dropout (Fixos: LR={lr_fixo:.5f}) ---")
    ...
    def wrapper(val_drop):
        ...
        best_accuracy = train_evaluate(lr=lr_fixo, dropout_rate=val_drop)
        print(
            f" Dropout Testado: {val_drop:.4f} -> Accuracy: {best_accuracy:.4f}")
...
best_lr = otimizar_lr(dropout_fixo=0.0)
...
best_drop = otimizar_dropout(lr_fixo=best_lr)
...
print(f"Config: LR={best_lr} | Drop={best_drop}")
